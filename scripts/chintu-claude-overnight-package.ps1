<#
.SYNOPSIS
    Generates the bounded Claude overnight prompt for Chintu OS.

.DESCRIPTION
    Writes a deterministic local Markdown handoff from fixed Stage 10 safety
    rules. It performs no network calls and does not read BALA app data.

.PARAMETER RepoRoot
    Chintu OS repository root. Defaults to the parent of this script folder.

.PARAMETER OutFile
    Prompt output path. Defaults to CHINTU_CLAUDE_OVERNIGHT_PROMPT.md in RepoRoot.
#>
[CmdletBinding()]
param(
    [string]$RepoRoot = "",
...
if (-not $OutFile) {
    $OutFile = Join-Path $RepoRoot "CHINTU_CLAUDE_OVERNIGHT_PROMPT.md"
}
...
Write-Host "Claude overnight prompt written: $OutFile"
Write-Host "Mode: local-only planning and hardening handoff"
exit 0
