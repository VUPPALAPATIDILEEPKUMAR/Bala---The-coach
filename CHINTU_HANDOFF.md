...
| `scripts/chintu-agent-board.ps1` | Local-only "agent board" (v1). Runs the release guard, reads the validation block, and writes a gitignored `chintu-agent-board-report.md` (optional `-OutFile`) with 8 local "agent" sections (Repo, Validation, Release Guard, BALA Safety, Privacy, PWA, Product, Founder Handoff) + next-3-sprints + Stop/Go. Not external bots; no push/install/network/secret/health-data. |
